...
import {
  Text, SafeAreaView, TouchableOpacity, StyleSheet, FlatList, Platform
} from 'react-native';
...
class StudyRoomsPreview extends Component {
    ...
    renderRow(item) {
      ...
      if (total.length > 0) { return (<TimeRangeCard title={item} available={total} hour={first.concat(third)} half={second.concat(fourth)} />); }
      return null;
    }
      ...
      return (
        <SafeAreaView style={styles.container}>
          {this.state.loading ? <Text> Loading </Text> : (
            <FlatList
              data={titles}
              extraData={this.state.listOfRooms}
